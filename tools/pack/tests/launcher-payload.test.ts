...
function createWinPaths(root: string, namespace: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Root Editor-release-beta-win-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Root Editor-release-beta-win.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Root Editor Beta"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Root Editor Beta", "Root Editor.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "Root Editor-release-beta-win-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "Root Editor-release-beta-win-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "Root Editor-release-beta-win-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Root Editor Beta", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "Root Editor.exe"),
...
async function writeFakeWinUnpackedApp(root: string, namespace: string, version: string): Promise<{
  ...
  await writeFile(join(paths.unpackedRoot, "Root Editor.exe"), "fake executable\n", "utf8");
  await writeFile(
  ...
  return {
    builtApp: {
      ...
      executablePath: join(paths.unpackedRoot, "Root Editor.exe"),
      source: "namespace",
...
describe("tools-pack launcher payload archives", () => {
  it("builds channel and namespace scoped payload manifests for both desktop platforms", () => {
    ...
    expect(buildMacLauncherPayloadManifest({
      ...
      appBundleName: "Root Editor Beta.app",
      channel: "beta",
      entry: {
        cwd: "payload/Root Editor Beta.app",
        executable: "payload/Root Editor Beta.app/Contents/MacOS/Root Editor Beta",
      },
      namespace: "release-beta",
      payloadRoot: "payload",
      platform: "darwin",
    ...
    expect(buildWinLauncherPayloadManifest({
      ...
      entry: {
        cwd: "payload",
        executable: "payload/Root Editor.exe",
      },
      namespace: "release-beta-win",
      payloadRoot: "payload",
      platform: "win32",
  ...
  it.skipIf(process.platform !== "darwin")("creates a mac payload zip with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.appBundleName).toBe("Root Editor Beta.app");
      expect(manifest.entry.executable).toBe("payload/Root Editor Beta.app/Contents/MacOS/Root Editor Beta");
  ...
  it.skipIf(process.platform !== "win32")("creates a Windows payload 7z with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.entry.executable).toBe("payload/Root Editor.exe");
      expect(manifest.version).toBe(version);
      await expectPathExists(join(extractRoot, "payload", "Root Editor.exe"));
