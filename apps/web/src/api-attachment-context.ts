...
async function buildApiAttachmentContext(
  ...
  for (let index = 0; index < attachments.length; index += 1) {
    ...
    if (remaining <= 0) {
      blocks.push(
        '[Root Editor omitted remaining attached files because the attachment context budget was exhausted.]',
      );
      break;
    }
...
function clipAttachmentText(text: string, maxChars: number): string {
  ...
  return `${text.slice(0, maxChars)}\n\n[Root Editor truncated ${omitted} chars from this attachment before sending it to the API provider.]`;
}
